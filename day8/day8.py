...
class Decoder:
    def __init__(self, patterns):
        ...
        self.mapping[1] = str_sort(list(filter(lambda pattern: len(pattern) == 2, patterns))[0])
        self.mapping[7] = str_sort(list(filter(lambda pattern: len(pattern) == 3, patterns))[0])
        self.mapping[4] = str_sort(list(filter(lambda pattern: len(pattern) == 4, patterns))[0])
        self.mapping[8] = str_sort(list(filter(lambda pattern: len(pattern) == 7, patterns))[0])
